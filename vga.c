// vga.c -- Defines functions for writing to the monitor.
//             heavily based on JamesM's kernel development tutorials,
//             but rewritten for Agustin Gutierrez.
...
static void scroll(void)
{
   ...
   u8int attributeByte = (FOREGROUND << 4) | (BACKGROUND & 0x0F);
...
void printk_unsigned(unsigned long number, s32int radix)
{
    char buffer[32];
    s32int pos = 0;
...
void printk_signed(long number, s32int radix)
{
    if (number < 0)
    {
        put('-');
...
{
    s32int state = PRINTK_STATE_NORMAL;
    s32int length = PRINTK_LENGTH_DEFAULT;
    s32int radix = 10;
    bool sign = false;
    bool number = false;
    ...
    while (*fmt)
    {
        switch (state)
        {
            ...
            PRINTK_STATE_SPEC_:
                switch (*fmt)
                {
                    case 'c':   put((char)va_arg(args, s32int));
                                break;
                ...
                if (number)
                {
                    if (sign)
                    {
                        ...
                        case PRINTK_LENGTH_DEFAULT:     printk_signed(va_arg(args, s32int), radix);
                                                        break;
                    ...
                    else
                    {
                        ...
                        case PRINTK_LENGTH_SHORT_SHORT:
                        case PRINTK_LENGTH_SHORT:
                        case PRINTK_LENGTH_DEFAULT:     printk_unsigned(va_arg(args, u32int), radix);
                                                        break;
